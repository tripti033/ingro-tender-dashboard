    ...
    <nav className="sticky top-0 z-50 bg-[#0D1F3C] text-white px-6 py-3 flex items-center justify-between shadow-md">
            ...
            return (
              <button
                ...
                className={`px-3 py-1.5 rounded text-sm transition-colors ${
                  isActive ? "bg-[var(--bg-card)]/20 font-medium" : "text-gray-300 hover:bg-[var(--bg-card)]/10"
                }`}
              >
                {link.label}
              </button>
      ...
      {user && (
        <div className="flex items-center gap-4">
          ...
          <button onClick={handleSignOut} className="text-sm bg-[var(--bg-card)]/10 hover:bg-[var(--bg-card)]/20 px-3 py-1.5 rounded transition-colors">
            Sign out
          </button>
        </div>
      )}
