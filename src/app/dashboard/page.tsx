...
import { getTenders, updateFlag, markAsRead, toggleRead, deleteTender, type Tender } from "@/lib/firestore";
...
function DashboardContent() {
  ...
  const handleDeleteDraft = async (e: React.MouseEvent, tender: Tender) => {
    e.stopPropagation();
    if (!confirm(`Delete this draft?\n\n"${(tender.title || tender.nitNumber).slice(0, 80)}"\n\nThis cannot be undone.`)) return;
    try {
      await deleteTender(tender.nitNumber);
      setTenders((prev) => prev.filter((t) => t.nitNumber !== tender.nitNumber));
    } catch (err) {
                  ...
                  {alertDrafts.map((t) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2.5 text-right">
                        <button
                          onClick={(e) => handleDeleteDraft(e, t)}
                          title="Delete draft"
